...
def img2npy(image_paths:list, process_id):
  print("Starting Process:{p} ,  works from {a} to {b}, {c} works in total".format(p=process_id, a=image_paths[0], b =image_paths[-1], c = len(image_paths)))
  cnt = 0
  for path in image_paths:
    print("\n>>>>{t}: starting work {cnt}/{total}: {p}".format(t=process_id, cnt=cnt, total =len(image_paths), p= path))
...
def multi_process_fill(thread_count, valid_image_paths):

  work_per_process = len(valid_image_paths)//thread_count + 1
  split_paths = [valid_image_paths[i: i + work_per_process] for i in range(0, len(valid_image_paths), work_per_process)]
  works = []
  for i, path in enumerate(split_paths):
    works.append((path, i))
  with multiprocessing.Pool(thread_count) as pool:
    results = [pool.apply_async(img2npy, p) for p in works]
    for r in results:
      print('\t', r.get())
