...
def Filter(graphpath, cdlpath):
    ...
    dataset = gdal.Open(graphpath[1], gdal.GA_ReadOnly)
...
# save_path_template2 = "/content/drive/MyDrive/SRTP_Datasets/DatasetsForML"
save_path_template2 ="C:/Users/11027/Downloads/dataset"
def FilterForML(locations, years):
    for location in locations:
        ...
        for year in years:
            paths = []
            for date in dates:
                path = load_path_template+"/"+str(location)+"/"+str(year)+"/S1_"+str(location)+"_"+str(year)+date+".tif"
                paths.append(path)
                cdlpath = load_path_template+"/"+str(location)+"/"+str(year)+"/CDL_"+ str(location)+"_"+str(year)+".tif"
            X, Y, nanfilter = Filter(paths,cdlpath)
            np.save(save_path_template2+"/"+str(location)+"/"+str(year)+"_x.npy",X)
            np.save(save_path_template2+"/"+str(location)+"/"+str(year)+"_y.npy",Y)
            np.save(save_path_template2+"/"+str(location)+"/"+str(year)+"_nanfilter.npy",nanfilter)
        print("year "+str(year)+" finished!")
